...
def diffs_to_levels_mora(df_all: pd.DataFrame, diffs: pd.DataFrame) -> pd.Series:
    base_date = pd.to_datetime(TRAIN_END)
    mora0 = float(df_all.loc[base_date, "Mora_total"])
    return mora0 + diffs["D_Mora_total"].cumsum()
...
def main():
    ...
    if "Mora_total" not in df_all.columns:
        raise ValueError("No existe columna Mora_total en el Excel.")
    ...
    levels["nocovid"] = diffs_to_levels_mora(df_all, y_nocovid)
    levels["macro_base"] = diffs_to_levels_mora(df_all, y_base)
    levels["macro_indep"] = diffs_to_levels_mora(df_all, y_ind)
    levels["macro_noindep"] = diffs_to_levels_mora(df_all, y_full)

    obs_hist = df_all.loc["2019-01-01":TRAIN_END, "Mora_total"].dropna()
    ...
    plt.title("Morosidad: niveles observados y escenarios contrafactuales")
    plt.ylabel("Mora_total (nivel)")
    plt.xlabel("Fecha")
    plt.legend(ncol=2)
    plt.grid(True, alpha=0.25)
    plt.tight_layout()

    out_png = os.path.join(OUT_DIR, "mora_levels_scenarios.png")
    plt.savefig(out_png, dpi=300)
    print("Gráfico guardado en:", out_png)

    out_csv = os.path.join(OUT_DIR, "mora_levels_scenarios.csv")
